 ...
 *
 * Recovery is "reload" only — once a render has thrown, the component's
 * state is unknown, so clearing the boundary's own error flag would just
 * re-render the same crashing subtree. A full reload is the honest fix;
 * we don't offer a fake "try again" that immediately re-throws.
 */
export default class ErrorBoundary extends Component<Props, State> {
  ...
  override render() {
    ...
    // A chunk-load failure has a recognizable message — a fresh fetch
    // usually resolves it (stale deploy, transient network blip), so the
    // copy nudges toward reload.
      ...
      <div className="errbound errbound-full">
        <div className="eb-card">
          ...
          <p>
            {isChunkError ? (
              <>
                catat couldn&rsquo;t load part of its code{surface ? ` for the ${surface} view` : ''}.
                This usually means the tab is stale after a new deploy — a reload fixes it.
              </>
            ) : (
              <>
                An unexpected error stopped {surface ? `the ${surface} view` : 'this screen'} from
                rendering. Your on-chain data is safe — nothing here writes to Walrus or Sui.
              </>
            )}
          </p>
